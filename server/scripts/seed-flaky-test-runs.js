...
// real data to compute against. Safe to re-run: it wipes and regenerates
// only the "Flaky Test Tracker Demo Suite" it owns (identified by the
// TITLE_PREFIX on its test cases), leaving every other suite/case/run alone.
//
// Usage: npm run seed:flaky -w server

import db from "../db.js";

const SUITE_NAME = "Flaky Test Tracker Demo Suite";
const SUITE_FEATURE = "flaky-tracker-demo";
const TITLE_PREFIX = "[Flaky Seed] ";
const NUM_RUNS = 30;
const RUN_CREATORS = ["ci-bot", "agata", "morgan"];

// Seeded PRNG (mulberry32) so re-running the script reproduces the same
// data instead of a different random sample every time.
...
const cleanup = db.transaction(() => {
  const existingSuite = db.prepare("SELECT id FROM test_suites WHERE name = ?").get(SUITE_NAME);
  if (existingSuite) {
    const runIds = db
  ...
  db.prepare("DELETE FROM test_cases WHERE title LIKE ?").run(`${TITLE_PREFIX}%`);
});
...
const seed = db.transaction(() => {
  ...
  `);
  const suiteId = insertSuite.run({ name: SUITE_NAME, feature: SUITE_FEATURE }).lastInsertRowid;
  ...
  for (let runIndex = 0; runIndex < NUM_RUNS; runIndex++) {
    // Most recent run is "now"; earlier runs step back roughly one day at a
    // time with jitter, so timestamps look like real CI history rather than
...
console.log(`Seeded "${SUITE_NAME}" with ${TEST_CASE_DEFS.length} test cases and ${NUM_RUNS} runs.\n`);
